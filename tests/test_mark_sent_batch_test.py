...
def test_single_entry_batch_works():
    ...
    db.write_market_entry(
        "TestSystem", "1", "TestStation", "Coriolis", "url", "Gold", 100
    )

    db.mark_sent_batch([("TestSystem", "TestStation", "Gold", "guild", "123")])

    data = db.read_all_entries()
    cooldown = data["TestSystem"]["stations"]["TestStation"]["metals"]["Gold"][
        "cooldowns"
    ]["guild"]["123"]
    assert cooldown > 0
...
def test_duplicate_entries_in_batch():
    ...
    db.write_market_entry(
        "DupSystem", "1", "DupStation", "Coriolis", "url", "Gold", 100
    )
    ...
    data = db.read_all_entries()
    cooldown = data["DupSystem"]["stations"]["DupStation"]["metals"]["Gold"][
        "cooldowns"
    ]["guild"]["123"]
    assert cooldown >= now
...
def test_mixed_recipient_types_in_batch():
    ...
    db.write_market_entry(
        "MixSystem", "1", "MixStation", "Coriolis", "url", "Gold", 100
    )
    ...
    assert (
        "guild"
        in data["MixSystem"]["stations"]["MixStation"]["metals"]["Gold"]["cooldowns"]
    )
    assert (
        "123"
        in data["MixSystem"]["stations"]["MixStation"]["metals"]["Gold"]["cooldowns"][
...
def test_batch_produces_identical_state_to_sequential():
    ...
    db_batch.write_market_entry(
        "System1", "1", "Station1", "Coriolis", "url", "Gold", 100
    )
    db_seq.write_market_entry(
        "System1", "1", "Station1", "Coriolis", "url", "Gold", 100
    )
...
def test_save_count_reduction():
    ...
    with count_save_calls(db_seq):
        db_seq.write_market_entry(
            "Test", "1", "Station", "Coriolis", "url", "Gold", 100
        )
        for i in range(10):
...
def test_missing_paths_handled_gracefully():
    ...
    db.write_market_entry(
        "ValidSystem", "1", "ValidStation", "Coriolis", "url", "Gold", 100
    )
    ...
    data = db.read_all_entries()
    cooldowns = data["ValidSystem"]["stations"]["ValidStation"]["metals"]["Gold"][
        "cooldowns"
    ]["guild"]
